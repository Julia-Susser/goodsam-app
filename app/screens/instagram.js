...
const pattern = date.compile('ddd, MMM D YYYY');
  data["data"].map((item) =>
    ...
    if (item.caption != undefined){
      w = <View style={styles.box}><Image source={{uri: item.media_url}} style={styles.image}/><Text>{datee}{"\n"}{item.caption}</Text></View>
      raw_list.push(w);
    }else{
      w = <View style={styles.box}><Image source={{uri: item.media_url}} style={styles.image}/><Text>{datee}</Text></View>
      raw_list.push(w);
    }
...
console.log(date.format(new Date(2020, 4, 5), pattern)); // => May 5 2020
